...
module biblioteka {
  'use strict';
  import IFieldError = biblioteka.errors.IFieldError;
  import ErrorService = biblioteka.errors.ErrorService;
  ...
  angular.module('biblioteka').config(httpInterceptorConfiguration);
    ...
    error400 = (rejection) => {
      if (rejection.data.exceptionType != null) {
        ...
        if (rejection.data.exceptionType.indexOf(
                "eu.programisci.biblioteka.common.security.authz.exceptions") != -1 ||
            rejection.data.exceptionType.indexOf(
                "eu.programisci.biblioteka.common.money.exceptions") != -1
        ) {
        ...
        if (rejection.data.exceptionType == "eu.programisci.biblioteka.common.recruitment.exceptions.CVSubmissionRequiredException") {
          this.JQueryUtilsService.showErrorMessage(
