...
module biblioteka {
  ...
  export class ModalUtilService implements IModalUtilService {
    ...
    public createProfileEditModal(user: biblioteka.ISlimUser): angular.IPromise<biblioteka.ISlimUser> {
      this.modal = this.$uibModal.open({
    ...
    public closeProfileEditModal(result: biblioteka.ISlimUser): void {
      if (!angular.isUndefined(this.modal) && this.modal != null) {
        this.modal.close(result);
        this.modal = null;
      }
  ...
  angular.module('biblioteka').service('ModalUtilService', biblioteka.ModalUtilService);
}